...
async fn make_tty_forwarder(local_tty: Zxio) -> Result<fidl::Channel, Error> {
    ...
    let signals_error = ZxioSignals::READ_DISABLED | ZxioSignals::PEER_CLOSED | ZxioSignals::ERROR;
    let signals_readable_or_error = ZxioSignals::READABLE | signals_error;

    std::thread::spawn(move || {
        let mut buf = [0; 1];

        loop {
            let (local_wait_handle, local_wait_signals) =
                local_tty.wait_begin(signals_readable_or_error.bits());
            let (remote_wait_handle, remote_wait_signals) =
                remote_tty.wait_begin(signals_readable_or_error.bits());
            ...
            let local_events =
                ZxioSignals::from_bits(local_tty.wait_end(wait_items[0].pending)).unwrap();
            let remote_events =
                ZxioSignals::from_bits(remote_tty.wait_end(wait_items[1].pending)).unwrap();

            if (local_events | remote_events).intersects(signals_error) {
                break;
            }

            if local_events.contains(ZxioSignals::READABLE) {
            ...
            if remote_events.contains(ZxioSignals::READABLE) {
